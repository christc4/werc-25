...
1) The attacker sends a deceptive email with malicious documents that look legitimate.

2) When executed on the target's machine, the attachment prompts the user to execute a macro or launch PowerShell to download the real payload.

3) Once this has been installed, the payload encrypts sensitive files, which can only be decrypted by paying the ransom.

4) Once encrypted, the payload displays a message saying that the user's files have been encrypted, and that they must pay a ransom to get them back.
